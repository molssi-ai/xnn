...
It is built directly from the papers' equations on the xnns abstractions
(:class:`~xnns.common.featurizers.GaussianRBF`,
:func:`~xnns.common.models.ops.scatter_sum`,
:func:`~xnns.common.models.ops.shifted_softplus`, ...); nothing is taken from
...
:class:`~xnns.common.models.outputs.ForceStressOutput` are smooth and
...
* ``cutoff_fn="cosine"`` multiplies the generated filter by a smooth
  :class:`~xnns.common.featurizers.CosineCutoff` envelope so the PES stays
  ...
  (``None``) reproduces.
* ``atom_ref``, a learnable per-element reference energy (the xnns
...
class _ShiftedSoftplus(nn.Module):
    ...
    :func:`~xnns.common.models.ops.shifted_softplus` (``ln(0.5 e^x + 0.5)``),
...
class SchNet(InteratomicPotential):
    ...
    gamma : float or None, optional
        ...
        spacing instead (see :class:`~xnns.common.featurizers.GaussianRBF`).
    cutoff_fn : str or None, optional
        ``"cosine"`` multiplies the generated filters by a smooth
        :class:`~xnns.common.featurizers.CosineCutoff` envelope (recommended
