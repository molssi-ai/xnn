...
no ``xnns`` import, no Python model code, no config file. That is what makes a
...
``forward_lammps(pos, edge_index, cell_shifts, atomic_numbers, cell)``
    The *pair-style* ABI, matching :class:`~xnns.common.deploy.LAMMPSWrapper`
...
-----------------------
:class:`~xnns.common.models.les.LatentEwald` adds an Ewald energy over latent
...
def _image_shifts(cell: Tensor, cutoff: float, pbc: Tensor) -> Tensor:
    ...
    :func:`~xnns.common.data.neighborlist._n_repeats` plus the
...
def build_neighbor_list_ts(pos: Tensor, cutoff: float, cell: Tensor,
    ...
    :func:`~xnns.common.data.build_neighbor_list` (same ``dst = i`` /
...
class _LatentEwaldHead(nn.Module):
    ...
    ``q_linear`` and the :class:`~xnns.common.models.les.EwaldSummation`), so
    ...
    Parameters
    ----------
    les : xnns.common.models.les.LatentEwald
    ...
    ewald : xnns.common.models.les.EwaldSummation
    ...
    def forward(self, features: Tensor, pos: Tensor, cell: Tensor,
        ...
        :meth:`~xnns.common.models.les.EwaldSummation.forward`.
...
class TorchScriptPotential(nn.Module):
    ...
    :class:`~xnns.common.models.les.LatentEwald`-wrapped) behind a fixed tensor
    ...
    def forward_lammps(self, pos: Tensor, edge_index: Tensor,
        ...
        Argument order matches :class:`~xnns.common.deploy.LAMMPSWrapper`, so
...
def export_torchscript_potential(model: nn.Module, cutoff: float, path: str,
    ...
    metadata, so a consumer can introspect the artifact without ``xnns``.
