...
Requires `ase` (optional extra: pip install "xnns[ase]").
"""
...
class XNNSCalculator(Calculator):
    """ASE Calculator wrapping a trained xnns model.
    ...
    ``pip install "xnns[ase]"``); constructing an instance raises
    ...
    **kwargs
    ...
    def __init__(self, model, cutoff: float, device: str = "cpu", **kwargs):
        if not _HAS_ASE:
            raise ImportError("ASE is required: pip install \"xnns[ase]\"")
