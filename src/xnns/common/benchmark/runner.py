...
``xnns train`` (or any other route) first.
...
class Benchmark:
    ...
    def _load_model(self, entry: ModelEntry):
        ...
        config (checkpoints written by :meth:`~xnns.common.train.Trainer.save`
        ...
        if entry.checkpoint is None:
            raise ValueError(
                ...
                f"(train one first with 'xnns train').")
        ...
        state = torch.load(entry.checkpoint, map_location=self.device,
                           weights_only=False)
        # Prefer the architecture embedded in the checkpoint (xnns-trained
    ...
    def _column_units(self) -> dict[str, str]:
        ...
        Units are labels only (xnns is unit-agnostic). Defaults are ``eV/atom``
