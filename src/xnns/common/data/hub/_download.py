...
``XNNS_DATASETS`` (or ``XNNS_CACHE``) environment variable, or per call with the
``cache_dir`` argument of :func:`~xnns.common.data.hub.load_dataset`.
"""
...
def _repo_datasets_dir() -> Path:
    ...
    Resolved relative to this file (``src/xnns/common/data/hub/_download.py``),
...
def default_cache_dir() -> Path:
    ...
    Honors the ``XNNS_DATASETS`` then ``XNNS_CACHE`` environment variables (both
    ...
    """
    for var in ("XNNS_DATASETS", "XNNS_CACHE"):
