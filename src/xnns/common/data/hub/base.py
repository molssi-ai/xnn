...
dataset into xnns' native *structure dicts* (the list-of-dicts format consumed
by :class:`~xnns.common.data.dataset.AtomicDataset`). Builders self-register
...
class DatasetBuilder(ABC):
    ...
    into xnns structure dicts. Register an instance with
    ...
    def load(self, *, split: Optional[str], cache_dir: Path,
        ...
        **kwargs
        ...
        dict of {str: list of dict} or list of dict
            ...
            :func:`~xnns.common.data.dataset.structure_to_graph`.
        """
...
def load_dataset(name: str, *, split: Optional[str] = None,
    ...
    Looks up the builder registered under ``name`` and returns its data as xnns
    structure dicts, optionally wrapped as ready-to-train
    :class:`~xnns.common.data.dataset.AtomicDataset` objects.
    ...
    :func:`~xnns.common.data.hub._download.default_cache_dir`.
