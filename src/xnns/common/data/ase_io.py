"""Read ASE-native structure data (extxyz, CIF, VASP, ...) into xnns.
...
:func:`~xnns.common.data.dataset.structure_to_graph` and
:class:`~xnns.common.data.dataset.AtomicDataset` consume. Training targets
...
ASE is an optional dependency (``pip install xnns[ase]``), so it is imported
...
def atoms_to_structure(atoms, energy_key: str = "energy",
    ...
    """Convert one ASE ``Atoms`` object to an xnns structure dict.
