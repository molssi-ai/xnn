...
``neural_network/NeuralNetwork.py`` and ``layers/``) on the xnns abstractions:
it subclasses :class:`~xnns.common.models.base.InteratomicPotential` directly
(PhysNet is a message-passing high-dimensional NN -- it has no hand-crafted
descriptor, so :class:`~xnns.dnn.models.base.DescriptorPotential` does not
apply) and gets forces/stress from the shared
:class:`~xnns.common.models.outputs.ForceStressOutput`. Given the same weights
...
electrostatics/dispersion over *all* pairs; in xnns the pair list is the
...
class PhysNet(InteratomicPotential):
    ...
    def forward(self, data: AtomicGraph) -> dict[str, Tensor]:
        ...
        dict of str to torch.Tensor
            ...
            xnns model, plus ``"charges"`` ``(N,)`` (corrected partial
    ...
    def from_config(cls, cfg) -> "PhysNet":
        ...
        spellings are translated by :mod:`xnns.common.config.translate`.
