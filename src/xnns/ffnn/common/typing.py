...
RDKit is an optional dependency (``pip install "xnns[ffnn]"``); everything
...
def _rdkit():
    try:
        ...
        raise ImportError(
            ...
            "pip install rdkit  (or the 'ffnn' extra of xnns)") from err
...
def assign_atom_types(structure, forcefield: Union[ForceField, str], *,
    ...
    forcefield : ForceField, str or object
        ...
        :func:`~xnns.ffnn.common.frc.read_forcefield` (``"oplsaa"``, a
        ``.frc`` path, ...), or any object exposing ``templates`` /
        ``fragments`` dicts in the same shape (an
        :class:`~xnns.ffnn.models.oplslib.OPLSLibrary` read from a ``.frc``
