...
structure (see :mod:`xnns.ffnn.common.typing`).
...
def builtin_data_dir() -> Path:
    """Directory of the ``.frc`` files shipped with xnns."""
    return Path(str(resources.files("xnns.ffnn").joinpath("data")))
...
class FrcFile:
    ...
    include_dirs : sequence of str or Path, optional
        ...
        including file's directory); the xnns data directory is always
        searched last.
...
def list_forcefields(include_dirs: Sequence[Union[str, Path]] = ()
    ...
    Scans the ``.frc`` files shipped with xnns (and any ``include_dirs``)
...
def find_forcefield(spec: str, include_dirs: Sequence[Union[str, Path]] = ()
    ...
    ----------
    spec : str
        One of: a variant name shipped with xnns (``"oplsaa"``, ``"CL&P"``,
