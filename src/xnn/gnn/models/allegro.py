...
:class:`~xnn.gnn.models.base.EquivariantGNN` (species bookkeeping, per-species
...
class Allegro(EquivariantGNN):
    ...
    atomic_energies, atomic_scales : array-like or None, optional
        Per-species energy shifts / scales (as in the xnn NequIP).
    """
    ...
    def node_features_energy(self, atomic_numbers: Tensor, edge_index: Tensor,
        ...
        tuple of torch.Tensor
            ...
            (what :class:`~xnn.common.models.les.LatentEwald` consumes) --
            and the per-atom energy ``(N,)``.
        """
        num_atoms = atomic_numbers.shape[0]
        center = edge_index[1]                     # xnn dst == allegro centre
    ...
    def from_config(cls, cfg) -> "Allegro":
        ...
        value forms coerced by :mod:`xnn.common.config.coerce`.
