...
BingqingCheng/cace reference code on the xnn abstractions: it subclasses
:class:`~xnn.gnn.models.base.GNNPotential` (species bookkeeping, per-species
energy shift ``atom_ref``), reuses the shared
:class:`~xnn.gnn.featurizers.BesselRBF` /
:class:`~xnn.gnn.featurizers.PolynomialCutoff` radial pieces and the
:class:`~xnn.gnn.featurizers.CartesianAngularBasis`, and adds the genuinely
...
Unlike the other xnn GNNs this model has no TorchScript/LAMMPS export path
...
class _SharedRadialTransform(nn.Module):
    ...
    l_of_entry : Tensor
        ...
        :class:`~xnn.gnn.featurizers.CartesianAngularBasis`).
    n_rbf : int
...
class CACE(GNNPotential):
    ...
    the xnn names at config-load time by
    ...
    def node_features_energy(self, atomic_numbers: Tensor, edge_index: Tensor,
        ...
        tuple of Tensor
            ...
            :class:`~xnn.common.models.les.LatentEwald` maps to latent
    ...
    def from_config(cls, cfg) -> "CACE":
        ...
        spellings are translated by :mod:`xnn.common.config.translate` and
        value forms coerced by :mod:`xnn.common.config.coerce`. Note that
