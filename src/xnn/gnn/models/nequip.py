...
abstractions: it subclasses :class:`~xnn.gnn.models.base.EquivariantGNN`
...
* the spherical harmonics are evaluated on ``r_j - r_i`` (neighbour minus
  centre) -- the opposite orientation to the xnn/MACE edge vector, so the
...
the LAMMPS/TorchScript exporters in :mod:`xnn.common.deploy`). The e3nn
...
class NequIP(EquivariantGNN):
    ...
    Subclasses :class:`~xnn.gnn.models.base.EquivariantGNN`, inheriting
    ...
    translated to the xnn names at config-load time by the key-translation
    registry in :mod:`xnn.common.config.translate`.
    ...
    def node_features_energy(self, atomic_numbers: Tensor, edge_index: Tensor,
        ...
        :class:`~xnn.common.data.AtomicGraph` dataclass. Embeds the species,
        runs the convnet layers, and reads out both the invariant
        conv-to-output features (what
        :class:`~xnn.common.models.les.LatentEwald` consumes) and the
    ...
    def from_config(cls, cfg) -> "NequIP":
        ...
        directly. Only the xnn canonical key names are read here; upstream
        ...
        if extra.get("global_rescale_scale") is not None:
            raise ValueError(
                "global_rescale_scale is not applied by the xnn NequIP; fold "
