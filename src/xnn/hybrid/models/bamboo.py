...
Molecular Simulation Booster, arXiv:2404.07181) built on the xnn
...
:class:`~xnn.transformer.attention.EdgeMultiheadAttention`), scales the
neighbour values by a radial edge feature (from the
:class:`~xnn.transformer.featurizers.ExpNormalSmearing` basis) and the
...
The model subclasses :class:`~xnn.common.models.base.InteratomicPotential`
directly (like :class:`~xnn.dnn.models.physnet.PhysNet`, the other
...
:class:`~xnn.common.models.outputs.ForceStressOutput` via autograd -- the
...
class GETLayer(nn.Module):
    ...
    multi-head :class:`~xnn.transformer.attention.EdgeMultiheadAttention` to
...
class BAMBOO(InteratomicPotential):
    ...
    :class:`~xnn.common.config.schema.ModelConfig` and the rest from its
    ...
    node_feature_dim : int
        ...
        :class:`~xnn.common.models.les.LatentEwald`.
    ...
    :class:`~xnn.common.models.les.LatentEwald` (BAMBOO exposes the required
    ...
    def forward(self, data: AtomicGraph) -> dict[str, Tensor]:
        ...
        dict of str to torch.Tensor
            ``"node_energy"`` ``(N,)`` and ``"energy"`` ``(B,)`` (as every xnn
    ...
    def from_config(cls, cfg) -> "BAMBOO":
        ...
        """
        from xnn.common.config.coerce import coerce_species
