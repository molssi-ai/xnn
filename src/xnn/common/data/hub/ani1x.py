...
Normal-Mode Sampling of :mod:`~xnn.common.data.hub.ani1`. This is the data the
ANI-1x *model* (:meth:`xnn.dnn.models.ani.ANI.ani1x`) was trained on, so it is
...
* Upstream energies are in **Hartree**, forces in **Hartree/angstrom**, and
  ...
  eV/angstrom (the convention used elsewhere in xnn); ``units="hartree"`` keeps
  the raw values.
* ``level`` selects the level of theory. ``"wb97x_dz"`` (default, wB97X/6-31G(d)
  ...
  ``load_dataset("ani1ccx")`` (see :mod:`~xnn.common.data.hub.ani1ccx`), which
...
class ANI1xBuilder(DatasetBuilder):
    ...
    then parsed into xnn structure dicts, keeping only the conformations for
