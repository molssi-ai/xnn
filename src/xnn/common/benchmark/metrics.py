...
def load_custom_metric(name: str, path: str) -> Metric:
    ...
    changes to xnn: give the dotted import path of a callable with the
...
def collect_predictions(model, loader, device, targets: list[str],
    ...
    :mod:`~xnn.common.benchmark.energy`). When ``energy_per_atom`` is set, the
    ...
    model : torch.nn.Module
        ...
        (e.g. a :class:`~xnn.common.models.ForceStressOutput`).
...
def score(pairs: dict[str, tuple[Tensor, Tensor]],
    ...
    metrics : dict of str to list of str, or list of str
        ...
        :attr:`~xnn.common.benchmark.BenchmarkConfig.metrics` form; targets
