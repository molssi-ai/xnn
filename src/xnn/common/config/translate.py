...
:class:`~xnn.common.config.schema.ModelConfig` fields plus each model's
...
inside the :func:`~xnn.common.config.loaders.from_dict` funnel -- so keys
...
# upstream spelling -> xnn canonical spelling, per model-registry name.
...
_KEY_TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    # the xnn SchNet itself is built from the papers, not from schnetpack
    "schnet": {
...
def register_key_translation(name: str, table: dict[str, str]) -> None:
    ...
    table : dict[str, str]
        Mapping of upstream key spellings to xnn canonical spellings; merged
